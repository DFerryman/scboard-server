...
TODAY_SIGNALS_SYSTEM_PROMPT = (
    ...
    "operators, or buyers. If previousInsight is present, use it only as "
    "anti-duplication context, not evidence: prefer today's fresh story "
    "signals and avoid repeating an older frame unless the routed evidence "
    "adds a materially new fact, stronger disagreement, or clearer buyer "
    "pain. If recentInsights is present, repeated frames must name the new "
    "evidence reason in the field itself or be avoided. Avoid generic hype, "
    "do not list stories, and do not "
...
OPPORTUNITY_SYSTEM_PROMPT = (
    ...
    "name specific buyer/user groups. If previousInsight is present, use it "
    "only as anti-duplication context, not evidence: prefer opportunity "
    "surfaces anchored in today's fresh story signals, and repeat an older "
    "thesis only when the candidate evidence shows a materially new buyer, "
    "risk, timing, or wedge. If recentInsights is present, every repeated "
    "surface must make the new evidence reason explicit. linkedStoryIds must cite the input "
...
DEBATE_SYSTEM_PROMPT = (
    ...
    "for builders or buyers. If previousInsight is present, use it only as "
    "anti-duplication context, not evidence: prefer disagreements grounded "
    "in today's fresh story signals, and revisit an older debate only when "
    "the candidate evidence changes the tradeoff, stakes, or observed "
    "behavior. If recentInsights is present, repeated debates must explain "
    "the new evidence reason. Security boundary: candidate discussion and "
...
TOPIC_SCOUT_SYSTEM_PROMPT = (
    ...
    "Use score, descendants, feedRanks, storyCount, recency, and novelty "
    "against previousInsight and recentInsights as ranking signals, but keep product relevance "
    "above raw popularity. If previousInsight is present, treat it only as "
    "anti-duplication context, not evidence: penalize repeated framings and "
    "select a previously covered topic only when fresh storySignals or "
    "metrics show a materially new angle, stronger buyer pain, or stronger "
    "disagreement. For repeated topics, the reason must state the new evidence "
    "reason. Do not exclude strong same-day evidence solely because the "
    "broad topic appeared before. "
